...
//		$this->load->module('template');
	...
	function detail()
	{
		...
		$col = "slug";
        $cat = $this->M_category->count_where($col, $slug_category);
        if ($cat === 1)
        {
        	$query_category = $this->M_category->get_where_custom($col, $slug_category);
	        ...
	        $query_products = $this->M_products->join_products($col, $slug_products);
	...
	function order()
	{
		...
		else
		{
			...
			$order_now = date("Ymd");
			$cek_invoice = $this->M_cart->custom_get_max('no_invoice', 'no_invoice', $order_now);
			$kode_invoice = "";
			...
			$insert = $this->M_cart->_insert($data);
	...
	function me()
	{
		...
		else
		{
			...
			// $this->template->header($data);
			$data = array (
				'query' => $this->M_products->get_where_custom('id_user', $id_user),
				'title' => 'Data Barang',
